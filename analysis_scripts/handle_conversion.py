...
data_folder = os.environ.get("TB_DATA")
calib_path = os.path.normpath(os.path.join(data_folder, '../ToT_TB2025_runs'))
...
available_path = os.path.join(data_folder, '/dut/module_0/chip_0')
...
file = 'config_summary_TB_GOODruns_google.csv'
df = pd.read_csv(file, sep=',')

corry_folder = os.environ.get("TB_ANALYSIS")
file = os.path.join(corry_folder, '/analysis_scripts/elog_extracted.csv')
df_elog = pd.read_csv(file, sep=',')


for calibration_avail in sorted(calib_run_numbers):
    print(calibration_avail)
...
for not_cal in not_calibrated_runs:
    ...
    if device == 'W8R6': 
        ...
        if (frontend in ['HVC']):
            ...
            print(f"{available_path}/run00{not_cal}_*_*_ext_trigger_scan_interpreted.h5")
